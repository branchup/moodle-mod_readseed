...
class machinegradingbyuser extends basereport
{
    ...
    public function process_raw_data($formdata)
    {
        ...
        $sql = "SELECT tai.id,tu.userid, tai.wpm, tai.accuracy,tu.timecreated,tai.attemptid, tai.sessionscore,tai.sessiontime,tai.sessionendword, tu.filename, tai.readseedid  FROM {" . constants::M_AITABLE . "} tai INNER JOIN  {" . constants::M_USERTABLE . "}" .
            " tu ON tu.id =tai.attemptid AND tu.readseedid=tai.readseedid WHERE tu.readseedid=? AND tu.userid=? ORDER BY 'tai.id DESC'";
        $alldata = $DB->get_records_sql($sql, array($formdata->readseedid,$formdata->userid));
