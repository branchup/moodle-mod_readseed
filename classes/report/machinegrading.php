...
class machinegrading extends basereport
{
    ...
    public function fetch_formatted_field($field, $record, $withlinks)
    {
        ...
        switch ($field) {
            ...
            case 'username':
                ...
                if ($withlinks) {
                    $link = new \moodle_url(constants::M_URL . '/grading.php',
                        array('action' => 'gradingbyuser', 'n' => $record->readseedid, 'userid' => $record->userid));
            ...
            case 'totalattempts':
                ...
                if ($withlinks) {
                    $link = new \moodle_url(constants::M_URL . '/grading.php',
                        array('action' => 'machinegradingbyuser', 'n' => $record->readseedid, 'userid' => $record->userid));
                    $ret = \html_writer::link($link, $ret);
                }
                break;
            ...
            case 'gradenow':
                if ($withlinks) {
                    $url = new \moodle_url(constants::M_URL . '/grading.php', array('action' => 'gradenow', 'n' => $record->readseedid, 'attemptid' => $record->attemptid));
            ...
            case 'regrade':
                ...
                if ($withlinks) {
                    $link = new \moodle_url(constants::M_URL . '/grading.php', array('action' => 'regradenow', 'n' => $record->readseedid, 'attemptid' => $record->attemptid));
            ...
            case 'review':
                ...
                if ($withlinks) {
                    $link = new \moodle_url(constants::M_URL . '/grading.php', array('action' => 'machinereview', 'n' => $record->readseedid, 'attemptid' => $record->attemptid));
            ...
            case 'deletenow':
                if ($withlinks) {
                    $url = new \moodle_url(constants::M_URL . '/manageattempts.php',
                        array('action' => 'delete', 'n' => $record->readseedid, 'attemptid' => $record->id, 'source' => $this->report));
    ...
    public function process_raw_data($formdata)
    {
        ...
        $sql = "SELECT tai.id,tu.userid, tai.wpm, tai.accuracy,tu.timecreated,tai.attemptid, tai.sessionerrors," .
            " tai.sessionscore,tai.sessiontime,tai.sessionendword, tu.filename, tai.readseedid,  u.firstnamephonetic," .
        "u.lastnamephonetic,u.middlename,u.alternatename,u.firstname,u.lastname  FROM {" . constants::M_AITABLE . "} tai INNER JOIN  {" . constants::M_USERTABLE . "}" .
            " tu ON tu.id =tai.attemptid AND tu.readseedid=tai.readseedid INNER JOIN {user} u ON tu.userid=u.id WHERE tu.readseedid=?" .
        " ORDER BY u.lastnamephonetic,u.firstnamephonetic,u.lastname,u.firstname,u.middlename,u.alternatename, tai.id DESC";
        $alldata = $DB->get_records_sql($sql, array($formdata->readseedid));
        ...
        if ($alldata) {
            foreach ($alldata as $thedata) {
                ...
                $thedata->audiourl = \mod_readseed\utils::make_audio_URL($thedata->filename, $formdata->modulecontextid, constants::M_COMPONENT,
                ...
                $errorcount = \mod_readseed\utils::count_sessionerrors($thedata->sessionerrors);
                $newerrorcount = $errorcount - $formdata->accadjust;
                if($newerrorcount < 0){$newerrorcount=0;}
                //recalculate scores with new errorcount
                $adjusted_scores = \mod_readseed\utils::processscores($thedata->sessiontime,
