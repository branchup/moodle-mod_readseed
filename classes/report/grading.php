...
class grading extends basereport
{
        ...
        switch ($field) {
            ...
            case 'gradenow':
                if ($withlinks) {
                    ...
                    $link = new \moodle_url(constants::M_URL . '/grading.php', array('action' => 'gradenow', 'n' => $record->readseedid, 'attemptid' => $record->id));
            ...
            case 'aigradenow':
                if ($withlinks) {
                    $link = new \moodle_url(constants::M_URL . '/grading.php', array('action' => 'aigradenow', 'n' => $record->readseedid, 'attemptid' => $record->id));
    ...
    public function process_raw_data($formdata)
    {
        ...
        $human_sql = "SELECT tu.*  FROM {" . constants::M_USERTABLE . "} tu INNER JOIN {user} u ON tu.userid=u.id WHERE tu.readseedid=?" .
        ...
        $moduleinstance = $DB->get_record(constants::M_TABLE,array('id'=>$formdata->readseedid));
        ...
        if($moduleinstance->machgrademethod==constants::MACHINEGRADE_MACHINE && $cantranscribe) {
                $alldata = $DB->get_records_sql($hybrid_sql, array($formdata->readseedid));
                if ($alldata) {
                ...
                $alldata =$DB->get_records_sql($human_sql, array($formdata->readseedid));
        }
