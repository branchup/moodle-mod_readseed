...
class comprehensiontest
{
    ...
    public function fetch_items()
    {
        global $DB;
        if (!$this->items) {
            $this->items = $DB->get_records(constants::M_QTABLE, ['readseed' => $this->mod->id],'itemorder ASC');
        }
        if($this->items){
            return $this->items;
        }else{
    ...
    public function fetch_latest_attempt($userid){
        ...
        $attempts = $DB->get_records(constants::M_USERTABLE,array('readseedid' => $this->mod->id,'userid'=>$userid),'id DESC');
        if($attempts){
    ...
    public function fetch_test_data_for_js(){
        ...
        foreach($items as $item) {
            ...
            //$testitem->resourceurl = $this->fetch_media_url(constants::AUDIOPROMPT_FILEAREA, $item);
            //$testitem->modelurl = $this->fetch_media_url(constants::AUDIOMODEL_FILEAREA, $item);
