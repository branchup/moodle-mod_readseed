...
/**
 * Internal library of functions for module readseed
 *
 * All the readseed specific functions, needed to implement the module
...
//creates a "unique" slide pair key so that backups and restores won't stuff things
  public static function create_rsquestionkey()
    ...
    public static function move_item($readseed, $moveitemid, $direction)
    {
        global $DB;
        ...
        if (!$items = $DB->get_records(constants::M_QTABLE, array('readseed' => $readseed->id), $sort)) {
    ...
    public static function delete_item($readseed, $itemid, $context)
    {
        global $DB;
        $ret = false;
        ...
        foreach ($fileareas as $filearea) {
            $fs->delete_area_files($context->id, 'mod_readseed', $filearea, $itemid);
        }
        $ret = true;
        return $ret;
    }
