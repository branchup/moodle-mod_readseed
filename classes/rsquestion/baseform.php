...
abstract class baseform extends \moodleform {
    ...
    public final function definition() {
        ...
        $mform->addElement('header', 'typeheading', get_string('createaitem', 'readseed', get_string($this->typestring, 'readseed')));
        ...
        if ($this->standard === true) {
            ...
            $mform->addElement('text', 'name', get_string('itemtitle', 'readseed'), array('size'=>70));
            $mform->setType('name', PARAM_TEXT);
            $mform->addRule('name', get_string('required'), 'required', null, 'client');

            $mform->addElement('editor', constants::TEXTQUESTION . '_editor', get_string('itemcontents', 'readseed'), array('rows'=>'4', 'columns'=>'80'), $this->editoroptions);
        ...
        $this->add_action_buttons(get_string('cancel'), get_string('saveitem', 'readseed'));
    ...
    protected final function add_textarearesponse($count, $label = null, $required = false) {
        if ($label === null) {
            $label = get_string('response', 'readseed');
        }
     ...
     * @return bool
     */
    public function construction_override($itemid,  $readseed) {
        return true;
    }
}