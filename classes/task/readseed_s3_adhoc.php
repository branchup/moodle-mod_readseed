...
/**
 * A mod_readseed adhoc task to fetch back transcriptions from Amazon S3
 *
 * @package    mod_readseed
 * @since      Moodle 2.7
 * @copyright  2015 Justin Hunt (poodllsupport@gmail.com)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class readseed_s3_adhoc extends \core\task\adhoc_task {
     ...
     */
	 public function execute(){
		...
		//$trace->output($cd->somedata)
         ...
         if(!\mod_readseed\utils::can_transcribe($activity)){
             $this->do_forever_fail('This activity does not support transcription',$trace);
             return;
         }

         $aigrade = new \mod_readseed\aigrade($cd->attemptid,$cd->modulecontextid);
         if($aigrade){
