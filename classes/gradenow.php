...
/**
 * Grade Now class for mod_readseed
 *
 * @package    mod_readseed
 * @copyright  2015 Justin Hunt (poodllsupport@gmail.com)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class gradenow{
	...
	function __construct($attemptid, $modulecontextid=0) {
	   ...
	   if($attemptdata){
			...
			$this->activitydata = $DB->get_record(constants::M_TABLE,array('id'=>$attemptdata->readseedid));
   ...
   public function get_next_ungraded_id(){
       ...
       $sql = "SELECT tu.*  FROM {" . constants::M_USERTABLE . "} tu INNER JOIN {user} u ON tu.userid=u.id WHERE tu.readseedid=?" .
           " ORDER BY u.lastnamephonetic,u.firstnamephonetic,u.lastname,u.firstname,u.middlename,u.alternatename,tu.id DESC";
       $records = $DB->get_records_sql($sql, array($this->attemptdata->readseedid));
       $past=false;
       $nextid=false;
   ...
   public function prepare_javascript($reviewmode=false,$force_aimode=false){
       ...
       $gradingopts['opts_id'] = 'mod_readseed_gradenowopts';
       ...
       $PAGE->requires->js_call_amd("mod_readseed/gradenowhelper", 'init', array(array('id'=>$gradingopts['opts_id'])));
