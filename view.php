...
/**
 * Prints a particular instance of readseed
 *
...
$n  = optional_param('n', 0, PARAM_INT);  // readseed instance ID - it should be named as the first character of the module

if ($id) {
    $cm         = get_coursemodule_from_id('readseed', $id, 0, false, MUST_EXIST);
    $course     = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
    $moduleinstance  = $DB->get_record('readseed', array('id' => $cm->instance), '*', MUST_EXIST);
} elseif ($n) {
...
$PAGE->set_url('/mod/readseed/view.php', array('id' => $cm->id));
require_login($course, true, $cm);
$modulecontext = context_module::instance($cm->id);

// Trigger module viewed event.
$event = \mod_readseed\event\course_module_viewed::create(array(
   'objectid' => $moduleinstance->id,
   'context' => $modulecontext
));
$event->add_record_snapshot('course_modules', $cm);
$event->add_record_snapshot('course', $course);
$event->add_record_snapshot('readseed', $moduleinstance);
$event->trigger();
...
$attempts = $DB->get_records(constants::M_USERTABLE,array('userid'=>$USER->id,'readseedid'=>$moduleinstance->id),'id DESC');
$ai_evals = \mod_readseed\utils::get_aieval_byuser($moduleinstance->id,$USER->id);

//can attempt ?
$canattempt = true;
$canpreview = has_capability('mod/readseed:preview',$modulecontext);
...
if($attempts && $retake==0){
    ...
    if(\mod_readseed\utils::can_transcribe($moduleinstance)) {
        $latest_aigrade = new \mod_readseed\aigrade($latestattempt->id, $modulecontext->id);
    }else{
    ...
    if( $have_humaneval || $have_aieval){
        ...
        switch($moduleinstance->humanpostattempt){
            ...
            case constants::POSTATTEMPT_EVAL:
                ...
                $gradenow = new \mod_readseed\gradenow($latestattempt->id,$modulecontext->id);
