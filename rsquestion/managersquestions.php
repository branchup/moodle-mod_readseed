 ...
 * replace i) MOD_readseed eg MOD_CST, then ii) readseed eg cst, then iii) rsquestion eg rsquestion
 *
...
require_capability('mod/readseed:itemedit', $context);

//set up the page object
$PAGE->set_url('/mod/readseed/rsquestion/managersquestions.php', array('itemid'=>$itemid, 'id'=>$id, 'type'=>$type));
$PAGE->set_title(format_string($readseed->name));
...
//we always head back to the readseed items page
$redirecturl = new moodle_url('/mod/readseed/rsquestion/rsquestions.php', array('id'=>$cm->id));
    ...
    if($action == 'confirmdelete'){
		...
		echo $renderer->header($readseed, $cm, 'rsquestions', null, get_string('confirmitemdeletetitle', constants::M_COMPONENT));
		echo $rsquestion_renderer->confirm(get_string("confirmitemdelete",constants::M_COMPONENT,$item->name),
			new moodle_url('/mod/readseed/rsquestion/managersquestions.php', array('action'=>'delete','id'=>$cm->id,'itemid'=>$itemid)),
			$redirecturl);
	...
	/////// Delete item NOW////////
    	...
    	require_sesskey();
		$success = \mod_readseed\rsquestion\helper::delete_item($readseed,$itemid,$context);
        redirect($redirecturl);
    }elseif($action=="moveup" || $action=="movedown"){
        \mod_readseed\rsquestion\helper::move_item($readseed,$itemid,$action);
        redirect($redirecturl);
    }
...
//get filechooser and html editor options
$editoroptions = \mod_readseed\rsquestion\helper::fetch_editor_options($course, $context);
$filemanageroptions = \mod_readseed\rsquestion\helper::fetch_filemanager_options($course,1);
...
switch($type){
	...
	case constants::TYPE_TEXTPROMPT_LONG:
		$mform = new \mod_readseed\rsquestion\textpromptlongform(null,
    ...
    case constants::TYPE_TEXTPROMPT_SHORT:
        $mform = new \mod_readseed\rsquestion\textpromptshortform();
        break;

    case constants::TYPE_TEXTPROMPT_AUDIO:
        $mform = new \mod_readseed\rsquestion\textpromptaudioform();
        break;
...
if ($data = $mform->get_data()) {
		...
		if(!$edit){
			...
			$theitem->{constants::TEXTQUESTION.'format'} = 0;
			...
			$theitem->rsquestionkey = \mod_readseed\rsquestion\helper::create_rsquestionkey();
			
			//try to insert it
			if (!$theitem->id = $DB->insert_record(constants::M_QTABLE,$theitem)){
					error("Could not insert readseed item!");
					redirect($redirecturl);
			}
		}			
		...
		$data = file_postupdate_standard_editor( $data, constants::TEXTQUESTION, $editoroptions, $context,
								'mod_readseed', constants::TEXTQUESTION_FILEAREA, $theitem->id);
		$theitem->{constants::TEXTQUESTION} = $data->{constants::TEXTQUESTION} ;
		$theitem->{constants::TEXTQUESTION.'format'} = $data->{constants::TEXTQUESTION.'format'} ;
    ...
    for($anumber=1;$anumber<=constants::MAXANSWERS;$anumber++){
        ...
        if(property_exists($data,constants::TEXTANSWER . $anumber . '_editor')) {
            $data = file_postupdate_standard_editor($data, constants::TEXTANSWER . $anumber, $editoroptions, $context,
                'mod_readseed', constants::TEXTANSWER_FILEAREA . $anumber, $theitem->id);
    ...
    //handle any filemanager the files
        //save audio prompt files
        if(property_exists($data,constants::AUDIOPROMPT)){
            file_save_draft_area_files($data->{constants::AUDIOPROMPT}, $context->id, constants::M_COMPONENT, constants::AUDIOPROMPT_FILEAREA,
                $theitem->id, $filemanageroptions);
        }

        //save audio model files
        if(property_exists($data,constants::AUDIOMODEL)){
            file_save_draft_area_files($data->{constants::AUDIOMODEL}, $context->id, constants::M_COMPONENT, constants::AUDIOMODEL_FILEAREA,
                $theitem->id, $filemanageroptions);
        }

        //save item picture files
        if(property_exists($data,constants::PICTUREPROMPT)){
            file_save_draft_area_files($data->{constants::PICTUREPROMPT}, $context->id, constants::M_COMPONENT, constants::PICTUREPROMPT_FILEAREA,
                $theitem->id, $filemanageroptions);
        }

		//now update the db once we have saved files and stuff
		if (!$DB->update_record(constants::M_QTABLE,$theitem)){
				print_error("Could not update readseed item!");
				redirect($redirecturl);
		}
...
if ($edit) {
    ...
    $mform->set_data($data);
    $PAGE->navbar->add(get_string('edit'), new moodle_url('/mod/readseed/rsquestion/rsquestions.php', array('id'=>$id)));
	...
	$mode='rsquestions';
	echo $renderer->header($readseed, $cm,$mode, null, get_string('edit', constants::M_COMPONENT));
	$mform->display();
	echo $renderer->footer();