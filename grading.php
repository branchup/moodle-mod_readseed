...
switch($action){
	case 'gradenowsubmit':
		$mform = new \mod_readseed\gradenowform();
		if($mform->is_cancelled()) {
			$action='grading';
			break;
		}else{
			$data = $mform->get_data();
			$gradenow = new \mod_readseed\gradenow($attemptid,$modulecontext->id);
			$gradenow->update($data);
			
			//update gradebook
            readseed_update_grades($moduleinstance, $gradenow->attemptdetails('userid'));
...
switch ($action){
	...
	case 'gradenow':
		...
		$gradenowform = new \mod_readseed\gradenowform(null,array('shownext'=>$nextid !== false));
    ...
    case 'regradenow':
        ...
        $aigrade = new \mod_readseed\aigrade($attemptid,$modulecontext->id);
        $aigrade->do_diff();
        ...
        if(has_capability('mod/readseed:manageattempts',$modulecontext )) {
    ...
    case 'machinegrading':
        $mode="machinegrading";
        $report = new \mod_readseed\report\machinegrading();
        //formdata should only have simple values, not objects
        //later it gets turned into urls for the export buttons
        $formdata = new stdClass();
        $formdata->readseedid = $moduleinstance->id;
        ...
        switch($moduleinstance->accadjustmethod){
            ...
            case constants::ACCMETHOD_AUTO:
                $accadjust = \mod_readseed\utils::estimate_errors($moduleinstance->id);
                break;
    ...
    case 'machinegradingbyuser':
        $mode = "machinegrading";
        $report = new \mod_readseed\report\machinegradingbyuser();
        //formdata should only have simple values, not objects
        //later it gets turned into urls for the export buttons
        $formdata = new stdClass();
        $formdata->readseedid = $moduleinstance->id;
...
$PAGE->requires->js_call_amd("mod_readseed/hiddenplayerhelper", 'init', array($aph_opts));
