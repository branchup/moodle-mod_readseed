...
/**
 * Define all the restore steps that will be used by the restore_readseed_activity_task
 */

/**
 * Structure step to restore one readseed activity
 */
class restore_readseed_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        // root element describing readseed instance
        $oneactivity = new restore_path_element(constants::M_MODNAME, '/activity/readseed');
        $paths[] = $oneactivity;

        //rsquestions
        $rsquestions = new restore_path_element(constants::M_QTABLE,
            '/activity/readseed/rsquestions/rsquestion');
        $paths[] = $rsquestions;
		...
		//attempts
		 $attempts= new restore_path_element(constants::M_USERTABLE,
                                            '/activity/readseed/attempts/attempt');
		$paths[] = $attempts;

		//airesults
        $airesults = new restore_path_element(constants::M_AITABLE,
            '/activity/readseed/attempts/attempt/airesults/airesult');
        $paths[] = $airesults;
    ...
    protected function process_readseed($data) {
        global $DB;
    ...
    protected function process_readseed_rsquestions($data) {
        global $DB;
	...
	protected function process_readseed_attempt($data) {
        global $DB;
    ...
    protected function process_readseed_ai_result($data) {
        global $DB;
    ...
    protected function after_execute() {
        ...
        $this->add_related_files(constants::M_COMPONENT, constants::AUDIOMODEL_FILEAREA, constants::M_QTABLE);
        $this->add_related_files(constants::M_COMPONENT, constants::PICTUREPROMPT_FILEAREA, constants::M_QTABLE);

        //do answer areas
        $this->add_related_files(constants::M_COMPONENT, constants::AUDIOANSWER_FILEAREA, constants::M_QTABLE);
