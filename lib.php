...
/**
 * Library of interface functions and constants for module readseed
 *
 * All the core Moodle functions, neeeded to allow the module to work
 * integrated in Moodle should be placed here.
 * All the readseed specific functions, needed to implement all the module
 ...
 */
function readseed_supports($feature) {
    switch($feature) {
 ...
 * whether the course reset functionality affects the readseed.
 *
 * @param $mform form passed by reference
 */
function readseed_reset_course_form_definition(&$mform) {
 ...
 * @return array
 */
function readseed_reset_course_form_defaults($course) {
    return array('reset_' . constants::M_MODNAME =>1);
}


function readseed_editor_with_files_options($context){
 ...
 */
function readseed_reset_gradebook($courseid, $type='') {
    global $CFG, $DB;
    ...
    if ($moduleinstances = $DB->get_records_sql($sql,$params)) {
        foreach ($moduleinstances as $moduleinstance) {
            readseed_grade_item_update($moduleinstance, 'reset');
        }
    }
}
 ...
 * readseed attempts for course $data->courseid.
 *
 * @global stdClass
 * @global object
 ...
 */
function readseed_reset_userdata($data) {
    global $CFG, $DB;
    ...
    if (!empty($data->{'reset_' . constants::M_MODNAME})) {
        ...
        if (empty($data->reset_gradebook_grades)) {
            readseed_reset_gradebook($data->courseid);
        }
 ...
 */
function readseed_grade_item_update($moduleinstance, $grades=null) {
    global $CFG;
    ...
    if(false && $moduleinstance->machgrademethod=constants::MACHINEGRADE_MACHINE &&
        ...
        $errorestimate = \mod_readseed\utils::estimate_errors($moduleinstance->id);
    }else{
        $errorestimate =0;
    }
 ...
 */
function readseed_update_grades($moduleinstance, $userid=0, $nullifnone=true) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');

    if ($moduleinstance->grade == 0) {
        readseed_grade_item_update($moduleinstance);

    } else if ($grades = readseed_get_user_grades($moduleinstance, $userid)) {
        readseed_grade_item_update($moduleinstance, $grades);

    } else if ($userid and $nullifnone) {
        $grade = new stdClass();
        $grade->userid   = $userid;
        $grade->rawgrade = null;
        readseed_grade_item_update($moduleinstance, $grade);
 ...
 */
function readseed_get_user_grades($moduleinstance, $userid=0) {
    global $CFG, $DB;
    ...
    $ai_sql = "SELECT u.id, u.id AS userid, ai.sessionscore AS rawgrade
                 ...
                 WHERE attempt.id= (SELECT max(id) FROM {". constants::M_USERTABLE ."} iattempt WHERE iattempt.userid=u.id AND iattempt.readseedid = ai.readseedid)  AND u.id = attempt.userid AND ai.readseedid = :moduleid
                       $user
    ...
    $human_sql = "SELECT u.id, u.id AS userid, a.sessionscore AS rawgrade
                     ...
                     WHERE a.id= (SELECT max(id) FROM {". constants::M_USERTABLE ."} ia WHERE ia.userid=u.id AND ia.readseedid = a.readseedid)  AND u.id = a.userid AND a.readseedid = :moduleid
                           $user
                  GROUP BY u.id";
...
function readseed_get_completion_state($course,$cm,$userid,$type) {
	return readseed_is_complete($course,$cm,$userid,$type);
}


//this is called internally only 
function readseed_is_complete($course,$cm,$userid,$type) {
	 global $CFG,$DB;
 ...
 *
 */
function readseed_dotask(progress_trace $trace) {
    $trace->output('executing dotask');
}

function readseed_get_editornames(){
	return array('passage','welcome','feedback');
}

/**
 * Saves a new instance of the readseed into the database
 *
 ...
 * of the new instance.
 *
 * @param object $readseed An object from the form in mod_form.php
 * @param mod_readseed_mod_form $mform
 * @return int The id of the newly inserted readseed record
 */
function readseed_add_instance(stdClass $readseed, mod_readseed_mod_form $mform = null) {
    global $DB;
...
function readseed_process_editors(stdClass $readseed, mod_readseed_mod_form $mform = null) {
	...
	$editors = readseed_get_editornames();
	$itemid=0;
	$edoptions = readseed_editor_no_files_options($context);
...
/**
 * Updates an instance of the readseed in the database
 *
 * Given an object containing all the necessary data,
 * (defined by the form in mod_form.php) this function
 ...
 */
function readseed_update_instance(stdClass $readseed, mod_readseed_mod_form $mform = null) {
    global $DB;
...
/**
 * Removes an instance of the readseed from the database
 *
 ...
 */
function readseed_delete_instance($id) {
    global $DB;

    if (! $readseed = $DB->get_record(constants::M_TABLE, array('id' => $id))) {
        return false;
    }

    # Delete any dependent records here #

    $DB->delete_records(constants::M_TABLE, array('id' => $readseed->id));
 ...
 */
function readseed_user_outline($course, $user, $mod, $readseed) {
 ...
 * @param stdClass $readseed the module instance record
 * @return void, is supposed to echp directly
 */
function readseed_user_complete($course, $user, $mod, $readseed) {
}

/**
 * Given a course and a time, this module should find recent activity
 * that has occurred in readseed activities and print it out.
 * Return true if there was output, or false is there was none.
 *
 * @return boolean
 */
function readseed_print_recent_activity($course, $viewfullnames, $timestart) {
 ...
 * {@link readseed_print_recent_mod_activity()}.
 *
 ...
 */
function readseed_get_recent_mod_activity(&$activities, &$index, $timestart, $courseid, $cmid, $userid=0, $groupid=0) {
}

/**
 * Prints single activity item prepared by {@see readseed_get_recent_mod_activity()}

 * @return void
 */
function readseed_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames) {
}
 ...
 * @return array
 */
function readseed_get_extra_capabilities() {
    return array();
}
...
/**
 * Is a given scale used by the instance of readseed?
 *
 * This function returns if a scale is being used by one readseed
 * if it has support for grading and scales. Commented code should be
 * modified if necessary. See forum, glossary or journal modules
 * as reference.
 *
 * @param int $readseedid ID of an instance of this module
 * @return bool true if the scale is used by the given readseed instance
 */
function readseed_scale_used($readseedid, $scaleid) {
    global $DB;

    /** @example */
    if ($scaleid and $DB->record_exists(constants::M_TABLE, array('id' => $readseedid, 'grade' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
...
/**
 * Checks if scale is being used by any instance of readseed.
 *
 * This is used to find out if scale used anywhere.
 *
 * @param $scaleid int
 * @return boolean true if the scale is used by any readseed instance
 */
function readseed_scale_used_anywhere($scaleid) {
    global $DB;
 ...
 */
function readseed_get_file_areas($course, $cm, $context) {
    return readseed_get_editornames();
}

/**
 * File browsing support for readseed file areas
 *
 * @package mod_readseed
 * @category files
 *
 ...
 */
function readseed_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    return null;
}

/**
 * Serves the files from the readseed file areas
 *
 * @package mod_readseed
 * @category files
 *
 ...
 * @param stdClass $context the readseed's context
 * @param string $filearea the name of the file area
 * @param array $args extra arguments (itemid, path)
 * @param bool $forcedownload whether or not force download
 * @param array $options additional options affecting the file serving
 */
function readseed_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
       global $DB, $CFG;
    ...
    if (!has_capability('mod/readseed:view', $context)) {
        return false;
    }
        ...
        $fullpath = "/$context->id/mod_readseed/$filearea/$itemid/$relativepath";
 ...
 * @param cm_info $cm
 */
function readseed_extend_navigation(navigation_node $navref, stdclass $course, stdclass $module, cm_info $cm) {
}
 ...
 */
function readseed_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $readseednode=null) {
}
